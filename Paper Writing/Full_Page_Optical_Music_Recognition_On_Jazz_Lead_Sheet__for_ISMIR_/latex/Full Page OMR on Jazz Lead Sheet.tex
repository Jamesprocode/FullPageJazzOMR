...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Full-Page omr for lead sheet.png}
...
All metrics are reported as edit-distance error rates, computed as the Levenshtein distance between the prediction and ground truth, divided by the total number of ground-truth units at the corresponding level and multiplied by 100. Following prior work in OMR and end-to-end music transcription~\cite{calvo-zaragoza_understanding_2021, rios-vila_sheet_2024}, we report Character Error Rate (CER), Symbol Error Rate (SER), and Line Error Rate (LER), which measure recognition accuracy at the character, symbol, and line levels, respectively. These metrics capture aspects of transcription quality: CER is sensitive to fine-grained token errors, SER reflects symbol-level correctness, and LER evaluates structural alignment. To better isolate melodic accuracy, these metrics are also computed on the \texttt{kern} spine only, separating note transcription from chord recognition. SER is additionally used as the primary signal for curriculum stage advancement.
...
\item \textbf{Root SER}: extracts only the root-note token from each chord symbol and computes the Levenshtein distance between the predicted and ground-truth root sequences, normalized by the total number of ground-truth root tokens. It isolates errors in root note recognition.
